...
{
    public const EXERCISE_NUMBER = 2;
    ...
    {
        // Distractors come from any unit word (variety); questions only from active.
        $allWords = $this->unit->words->pluck('id')->all();
    ...
    {
        $service->markExerciseCompleted(auth()->id(), $this->unit->id, 2);

        session()->flash('exercise_completed', '¡Ejercicio "Multichoice in English" completado!');
...
<div>
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight truncate">
                    Multichoice English · Unidad {{ $unit->unit_number }}
                </h2>
    ...
    <div class="py-6 sm:py-10">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            @if ($finished)
                @php
                    $score = $this->total > 0 ? (int) round(($correct / $this->total) * 100) : 0;
                @endphp
                ...
                <div wire:key="mcq-{{ $word->id }}"
                    ...
                    <div class="p-6 sm:p-8">
                        <div class="text-center mb-6">
                            <p class="text-xs uppercase tracking-wide font-semibold {{ $palette['text'] }} mb-2">¿Qué palabra significa?</p>
                            <p class="text-lg sm:text-xl text-gray-800 dark:text-gray-200 font-medium leading-snug">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            @foreach ($this->options as $opt)
                                <button wire:click="answer({{ $opt->id }})"
                                        @disabled($answered)
                                        class="px-4 py-4 rounded-lg border-2 font-semibold text-base transition-all {{ $this->optionClass($opt->id) }}">
                        ...
                        @if ($answered && $word->example)
                            <div class="mt-5 p-4 rounded-lg {{ $palette['soft'] }}">
                                <p class="text-xs uppercase tracking-wide font-bold {{ $palette['text'] }} mb-1">Ejemplo</p>
                                <p class="text-sm italic text-gray-800 dark:text-gray-200">"{{ $word->example }}"</p>
                            </div>
                        @endif
